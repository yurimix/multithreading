...
public class AbstractPool<T> {
    ...
    private synchronized T acquireObject() {
        return this.poolObjects.stream().filter(p -> !p.isIssued()).findFirst().map(this::issueObject)
                .map(PoolObject::getData).orElseThrow(IllegalStateException::new);
    }

    private synchronized void releaseObject(T po) {
        this.poolObjects.stream().filter(PoolObject::isIssued).filter(p -> Objects.equals(p.getData(), po)).findFirst()
                .map(this::returnObject).orElseThrow(IllegalArgumentException::new);
    }
    ...
    public List<String> statistics() {
        return poolObjects.stream().map(po -> po.getData().toString() + ", issued: " + po.isIssued())
